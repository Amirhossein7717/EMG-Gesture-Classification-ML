"""Build AItools_project_final.ipynb from the cleaned Python source."""
import json
...
cells.append(md([
    ...
    "8. Performance Report and Comparison\n",
    "9. Feature Importance and GridSearchCV Heatmap\n",
]))
...
cells.append(code([
    ...
    "# pip install antropy xgboost scikit-learn pandas numpy matplotlib seaborn scipy\n\n",
    "import numpy as np\n",
    "import pandas as pd\n",
...
cells.append(md([
    "## Step 2 — Load Data\n\n",
    "Place `RockData.csv` and `PaperData.csv` in the same directory as this notebook.",
]))
cells.append(code([
...
# ── Step 3: Cleaning ───────────────────────────────────────────────────────────
cells.append(md(["## Step 3 — Data Cleaning (Tasks 2, 3, 5, 6)"]))
cells.append(code([
    "X_raw = data.drop(columns=['label'])\n",
    "y     = data['label'].values\n\n",
    "X_raw = X_raw.apply(pd.to_numeric, errors='coerce')\n\n",
    "# Task 5: Missing value check\n",
...
cells.append(md(["## Step 4 — Data Visualisation (Task 4)"]))
cells.append(code([
...
cells.append(md([
    "## Step 5 — Feature Extraction (Task 7)\n\n",
    "Each trial (1024 samples) is summarised into **26 features** across four domains.\n\n",
    "> **Note on filtering:** According to `DatasetDescription.docx`, the EMG signals were already\n",
    "> filtered before recording. No additional filtering is applied here.",
]))
cells.append(code([
...
cells.append(code([
    "# Run extraction across all trials (~3-5 minutes)\n",
...
cells.append(md([
    "## Step 6 — Feature Selection (Task 8)\n\n",
    "**Pipeline:** Standardise → Kruskal-Wallis H-test (p < 0.05) → PCA (95% variance retained)",
]))
cells.append(code([
    "scaler      = StandardScaler()\n",
    "X_scaled    = scaler.fit_transform(features_df)\n",
    "X_scaled_df = pd.DataFrame(X_scaled, columns=features_df.columns)\n",
    "print(f'Standardised feature matrix: {X_scaled_df.shape}')",
]))
cells.append(code([
...
cells.append(code([
    ...
    "for cls, color, lbl in [(0, 'steelblue', 'Rock'), (1, 'coral', 'Paper')]:\n",
    "    mask = y == cls\n",
    "    plt.scatter(X_pca2d[mask, 0], X_pca2d[mask, 1],\n",
    "                c=color, label=lbl, alpha=0.4, s=10)\n",
...
cells.append(md(["## Step 7 — Classification: Hold-out Validation (Tasks 9, 10, 11)"]))
cells.append(code([
...
# ── Step 8: Confusion + ROC ────────────────────────────────────────────────────
cells.append(md(["## Step 8 — Confusion Matrices and ROC Curves (Task 11)"]))
cells.append(code([
...
cells.append(md(["## Step 9 — Performance Report (Task 11)"]))
cells.append(code([
    ...
    "print(classification_report(y_test, holdout_results[best_clf_name]['y_pred'],\n",
    "                             target_names=['Rock', 'Paper']))",
]))

# ── Step 10: Cross-Validation ──────────────────────────────────────────────────
cells.append(md(["## Step 10 — 5-Fold Cross-Validation (Task 12)"]))
cells.append(code([
    ...
    "plt.boxplot([cv_results[n] * 100 for n in cv_results],\n",
...
cells.append(md(["## Step 11 — GridSearchCV Hyperparameter Tuning (Task 12)"]))
cells.append(code([
...
cells.append(md(["## Step 12 — Final Comparison: Hold-out vs CV vs GridSearchCV (Task 13)"]))
cells.append(code([
    "comparison = []\n",
    ...
    "    grid_acc    = (accuracy_score(y_test, y_pred_gs) * 100\n",
    "                   if name == 'Random Forest' else 'N/A')\n",
    "    comparison.append({\n",
    "        'Classifier'         : name,\n",
...
# ── Step 12b: Feature Importance ──────────────────────────────────────────────
cells.append(md([
    "## Step 12b — Feature Importance Analysis\n\n",
    "A separate Random Forest is fitted on the Kruskal-Wallis selected features (before PCA) ",
    "so that importances map back to interpretable feature names.",
]))
cells.append(code([
...
cells.append(md([
    "## Step 12c — GridSearchCV Accuracy Heatmap\n\n",
    "Heatmap of 5-fold CV accuracy across `n_estimators` x `max_depth` at `min_samples_split = 2`.",
]))
cells.append(code([
...
cells.append(md([
    "## Step 13 — Results Interpretation and Discussion (Task 14)\n\n",
    "| Property | Value |\n",
    "|---|---|\n",
    "| Classes | Rock (0) vs Paper (1) |\n",
    "| Total trials | 5,850 (2,909 Rock + 2,941 Paper) |\n",
    "| Samples per trial | 1,024 (~4 s at 256 Hz) |\n",
    "| Channels | 1 (single-channel EMG) |\n",
    "| Missing values | None |\n",
    "| Filtering | Not applied (signals pre-filtered) |\n\n",
    "**Best model:** SVM (RBF) — hold-out accuracy 78.97%, F1 80.63%, AUC 0.875  \n\n",
    "**Feature insight:** Multi-domain fusion (79.49%) outperforms every single domain. ",
    "Entropy features (75.04%) are the most informative single group; frequency-domain (68.97%) the weakest.  \n\n",
    "**Validation:** 5-fold CV provides the most reliable estimate. ",
    "GridSearchCV (18 configurations, 90 fits) confirmed default RF hyperparameters were near-optimal.  \n\n",
    "**SOTA context:** Oskoei and Hu (2007) reported SVM accuracies of 80-95% on multi-channel (6-10 ch) ",
    "upper-limb EMG; on comparable single-channel setups Phinyomark et al. (2012) documented ",
    "time-domain feature accuracies in the 70-85% range, consistent with the 78.97% achieved here.  \n\n",
    "**Limitations:** Single channel only; binary task; no subject-independent (LOSO) evaluation.",
]))
...
with open("AItools_project_final.ipynb", "w", encoding="utf-8") as f:
    json.dump(nb, f, ensure_ascii=False, indent=1)

print(f"Notebook written: AItools_project_final.ipynb  ({len(cells)} cells)")
